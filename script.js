document.addEventListener('DOMContentLoaded', () => {
    ...
    const items = {
        ...
        'spider silk': { name: 'Spider Silk', description: 'A clump of strong, sticky spider silk.', use: (f) => {
            if(gameState.currentRoom === 'alchemistStudy' && gameState.flags.alchemistQuest === 'accepted'){
                removeItem('spider silk'); addItem('potion of regeneration', f); gameState.flags.alchemistQuest = 'completed';
    ...
    const showRoom = (roomId) => {
        ...
        if (PATROL_ZONES.includes(roomId) && gameState.currentRoom !== roomId && Math.random() < RANDOM_ENCOUNTER_CHANCE) {
        ...
        const endCombat = () => {
            ...
            if (enemy.loot) {
                if (addItem(enemy.loot, feedback)) {
                    ...
                    // This function doesn't take feedback, so we add the message manually
                    if (!gameState.roomStates[postCombatRoomId]) {
                        gameState.roomStates[postCombatRoomId] = { droppedItems: [] };
                    }
                    gameState.roomStates[postCombatRoomId].droppedItems.push(enemy.loot);
    ...
    loadBtn.addEventListener('click', () => {
        ...
        if (savedState) {
            if (confirm('Load your saved game?')) {
                ...
                // Ensure gameState has new properties if loading an old save
                if (!gameState.roomStates) gameState.roomStates = {};
                alert('Game Loaded!');
                showRoom(gameState.currentRoom);
            }
